 ...
 * - TODO APP under test: https://todo-app.serenity-js.org/#/
 */
...
describe("Serenity/JS Website", () => {
  ...
  it("offers a web testing tutorial", async () => {
    // Actor representing people and external systems interacting
    // with the system under test and playing specific roles.
    //   -- NOTE: actors attempt to perform interactions
    await actorCalled("Alice(Basic Example)").attemptsTo(
      // ----- ACTIVITIES
      ...
      // Abilities also enable actors to answer questions about the state of the system under test.
      // Questions provide a way to parameterise activities and allow actors to retrieve information when the activity is performed.
      // More about Web Assertions: https://serenity-js.org/handbook/design/assertions/#web-assertions
      Ensure.that(
  ...
  it(`offers examples to help you practice test automation`, async () => {
    ...
    await actorCalled("John(API Tester)").attemptsTo(
    ...
    await actorCalled("Wendy(Acceptance Tester)").attemptsTo(
